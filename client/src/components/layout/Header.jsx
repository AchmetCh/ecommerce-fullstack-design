...
const Header = () => {
  const { cartItems,role, user, logout, searchQuery, handleSearch, handleCategorySelect } = useAuth();
  ...
  const handleSumQuantities = () => {
    // Handle quantity logic here if needed
    ...
    <>
      ...
      <div className="bg-gray-800 text-white py-2">
        <div className="max-w-7xl mx-auto px-4 flex justify-between items-center">
          ...
          <div className="flex space-x-4 text-sm">
            {user ? (
              <>
                <span>Hello, {user.role || user.email}</span>
      ...
      <header className="bg-white shadow-md p-4">
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row items-center justify-between gap-4">
          ...
          {/* Search Bar */}
            <form onSubmit={handleSearchSubmit} className="relative pl-4 w-full">
              <input
                type="text"
                placeholder="Search products..."
                value={localSearch}
                onChange={(e) => handleSearchInput(e.target.value)}
                className="w-96 gap-4 pl-4 px-4 py-3 pr-12 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:border-blue-500 focus:ring-blue-200"
              />
              <button
                type="submit"
